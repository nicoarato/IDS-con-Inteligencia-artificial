%Entrenamiento para perceptrón multicapa con término de momento
...
function [wcell]=entrenamiento_multicapa_momento(X,yd,cant_c,max_epocas,alfa_sigm,gamma,umbral_acierto,t_momento,wcell)
    ...
    while(c_epocas<max_epocas) 
        for p=1:m %recorro cada patrón
            ...
            %RETROPROPAGACIÓN HACIA ATRÁS (vuelta) <<<-- :|

            %Delta de error total de la red (ultima capa)
            e=yd(p,:)-y;
            dcell{1,cant_c}=[e*1/2*(1-y)*(1+y)];

            %Delta de error de las demás capas de la red
            ...
            if(p==1)                            %El primero patrón lo actualizo sin término de momento
            ...
            else                                %Los demás patrones los actualizo con término de momento
                w_aux=wcell(1,:);
                for c=1:cant_c
                     dc=dcell{1,c};             %Deltas de error de las neuronas de la capa c (vector = cantidad neuronas de la capa)
                     Xc=[-1 ;ycell{1,c}];       %Entradas de la capa c (Vector columna)
...
%             graficar_ejercicio3(X,yd,wcell,cant_c,alfa_sigm);title('Entrenamiento CON TÉRMINO DE MOMENTO');
%             figure(2);
%         end
